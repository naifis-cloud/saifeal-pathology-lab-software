...
public partial class list_users : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                lbUserRights = BusinessLogic.GetUserAccessRights(Convert.ToInt32(Session["RoleId"]), 35, "View");

                if (!lbUserRights)
                {
                    lblMessage.Text = "You are not authorised to view users.";
            ...
            // if logged in as ColCenter SubCol center OR Referral doctor, do not allow to view collection center//
    ...
    private void GetAllUsers()
    {
        ...
        try
        {
            lobjdtDetails = bllogic.GetAllUsers(0, 0, "U.dtCreationDate DESC", "");
            rptUsers.DataSource = lobjdtDetails;
            rptUsers.DataBind();
    ...
    protected void rptUsers_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        try
        {
            ...
            if (e.CommandName == "eDelete")
            {
                ...
                if (liRoleFlag == 0 && !BusinessLogic.GetUserAccessRights(Convert.ToInt32(Session["RoleId"]), 35, "Delete"))
                    Commonfunction.showMsg("You are not authorised to delete a user.", this);
                else
                {
                    liRetVal = bllogic.DeleteUser(Convert.ToInt32(e.CommandArgument));
                    if (liRetVal > 0)
                    {
                        Commonfunction.showMsg("User deleted successfully.", this);
                        GetAllUsers();
                    }
                }
            }
            ...
            {
                lobjData = bllogic.GetUserDetails(Convert.ToInt32(e.CommandArgument));

                if (lobjData.Rows.Count > 0)
                {
                    EmailPassword(Convert.ToString(lobjData.Rows[0]["stName"]), Convert.ToString(lobjData.Rows[0]["stUserName"]),
                        Convert.ToString(lobjData.Rows[0]["stEmail"]), Convert.ToString(lobjData.Rows[0]["stPassword"]));
    ...
    private void EmailPassword(string fstrName, string fstrUsername, string fstrEmail, string fstrPassword)
    {
        ...
        lstrBuilder.Append("Dear " + fstrName + " ,<br /><p>Thank you for requesting your login details. Please find the below login details:</p></td></tr>");
        ...
        Commonfunction.SendMail(fstrEmail, Convert.ToString(ConfigurationManager.AppSettings["Support"]), "SHC - User Login Details", Convert.ToString(lstrBuilder));
    }
}