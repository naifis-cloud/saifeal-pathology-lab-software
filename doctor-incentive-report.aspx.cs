...
public partial class doctor_incentive_report : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                lbUserRights = BusinessLogic.GetUserAccessRights(Convert.ToInt32(Session["RoleId"]), 34, "View");

                if (!lbUserRights)
                {
                    lblMessage.Text = "You are not authorised to view doctor's incentive report.";
    ...
    private void GetSummaryReport()
    {
        ...
        try
        {
            ...
            lobjdtDetails = bllogic.GetDoctorIncentiveReport(lstrFromDate, lstrToDate, liNoOfDays);
            rptDoctorIncentive.DataSource = lobjdtDetails;
            rptDoctorIncentive.DataBind();

            if (lobjdtDetails.Rows.Count > 0)
            {
                rptDoctorIncentive.Visible = true;
                trNoData.Visible = false;
            }
            else
            {
                rptDoctorIncentive.Visible = false;
    ...
    private void ExportData()
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=DoctorIncentiveReport.xls");
        Response.Charset = "";
        Response.ContentType = "application/ms-excel";
        StringWriter sw = new StringWriter();
        HtmlTextWriter hw = new HtmlTextWriter(sw);
        rptDoctorIncentive.RenderControl(hw);
    ...
    private void ExportPDFData()
    {
        ...
        StringWriter sw = new StringWriter();
        HtmlTextWriter hw = new HtmlTextWriter(sw);
        pnlIncentiveReport.RenderControl(hw);
